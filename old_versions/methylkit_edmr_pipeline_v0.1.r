...
#get all pairwise combinations to create DMRs across
aa=combn(a,2)

for(i in 1:(length(aa)/2)){
#read bismark sam file output(s) into methylKil to get MethylRawList data format
myobj=read.bismark(list(aa[1,i],aa[2,i]),sample.id=list(strsplit(aa[1,i],'_')[[1]][1],strsplit(aa[2,i],'_')[[1]][1]),assembly='Bd21Control',save.context=context,read.context=context,treatment=c(0,1),mincov=min.cov)
